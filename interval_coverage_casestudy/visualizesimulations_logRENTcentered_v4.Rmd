---
title: "Simulation results when response is log(RENT)"
...
## Prior/Posterior predictive check for $J = 12$ income brackets where response is RENT, for nonrepresentative dataset of size 500, $D_3$
...
## Calculating $\hat{\mathbb{V}}_{\hat{\theta} \sim p(\theta \mid \mathcal{D}), \left(Z_1^S,\dots,Z_N^S \right)} \left( \hat{\mu} \right) = \frac{1}{N^2}  \text{tr} \left( {\Sigma_{\theta \mid \mathcal{D}}} \hat{{\Sigma}}_{J,N} \right) + \frac{1}{N^2} \vec{\hat{N}}^T {\Sigma_{\theta \mid \mathcal{D}}} \vec{\hat{N}}   + \frac{1}{N^2} {\hat{\Theta}_{J}}^T \hat{{\Sigma}}_{J,N} {\hat{\Theta}_{J}}$:

We will use $\mathbb{E}_{\hat{\theta} \sim p(\theta \mid \mathcal{D}), \left(Z_1^S,\dots,Z_N^S \right)} \left( \hat{\mu} \right)  \pm 1.96 \sqrt{\hat{\mathbb{V}}_{\hat{\theta} \sim p(\theta \mid \mathcal{D}), \left(Z_1^S,\dots,Z_N^S \right)} \left( \hat{\mu} \right)}$ to build an approximate 95 percent uncertainty interval around the population mean.

### Response is log(RENT)

200 simulation iterations were run:
...
#   #                   colour=Method,
#   #                   group=Method)) +
#   #     facet_grid(rows = vars(Dsize), cols = vars(`Income Brackets`)) + 
#   #     ggtitle("") +
#   #     theme_minimal() +
#   #     theme(legend.position="bottom",
#   #           legend.box="vertical",
...
#   #     ) + ylab("Absolute Bias") +
#   #     xlab("ACS size") +  
#   #     scale_colour_manual(values=c(`Sample Mean point estimates`="#4575b4",
#   #                                  `Poststratified Sample Mean point estimates`="#d73027",
#   #                                  `MRP point estimates`="#fdae61"))
#   # )
#   # ggsave(paste0("subpop_bias_D", k, ".png"), width = 18, height = 12)
#   # 
#   # 
#   # plot(
#   #   ggplot(
#   #     data = var_subpop_facetdf %>% 
#   #       filter(
#   #         datatype == paste0("D",k),
#   #         `Income Brackets`!="18 groups"
#   #       )) + 
...
#   #                                  `MRP point estimates`="#fdae61"))
#   # )
#   # ggsave(paste0("subpop_var_D", k, ".png"), width = 18, height = 12)
#   # 
#   
#   plot(
#     ggplot(
...
#                                    `MRP point estimates`="#fdae61"))
#   )
#   ggsave(paste0("subpop_mse_D", k, ".png"), width = 18, height = 12)
#   
#   # pop mse, bias, var ####
#   #   plot(
#   #   ggplot(
...
#   #                                  `MRP point estimates`="#fdae61"))
#   # )
#   # ggsave(paste0("bias_D", k, ".png"), width = 18, height = 12)
#   # 
#   # 
#   # plot(
#   #   ggplot(
...
#   #                                  `MRP point estimates`="#fdae61"))
#   # )
#   # ggsave(paste0("var_D", k, ".png"), width = 18, height = 12)
#   # 
#   
#   plot(
#     ggplot(
...
#       #                              `MRP point estimates`="#fdae61"))
#   )
#   ggsave(paste0("mse_D", k, ".png"), width = 18, height = 12)
#   
#   
#   
#   
